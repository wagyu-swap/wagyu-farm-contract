...
contract VlxStaking is Ownable {
    ...
    // WVLX
    address public immutable WVLX;

    // WAGYUe tokens created per block.
    ...
    // limit 10000 VLX here
    uint256 public limitAmount = 10000000000000000000000;
    // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when WAGYUe mining starts.
    uint256 public startBlock;
    // The block number when WAGYUe mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 wagyuReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function deposit() public payable {
        ...
        if(msg.value > 0) {
            IWVLX(WVLX).deposit{value: msg.value}();
            assert(IWVLX(WVLX).transfer(address(this), msg.value));
    ...
    function safeTransferVLX(address to, uint256 value) internal {
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            IWVLX(WVLX).withdraw(_amount);
            safeTransferVLX(address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accWagyuPerShare).div(1e12);
